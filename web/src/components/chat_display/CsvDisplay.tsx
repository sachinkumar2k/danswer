...
export default function CsvPage({ csvFileDescriptor, close }: { csvFileDescriptor: FileDescriptor, close: () => void }) {
  ...
  return (
    <>
      {expanded ?
        <Modal onOutsideClick={() => setExpanded(false)} className='!max-w-5xl rounded-2xl animate-all ease-in !p-0'>
...
export const CsvSection = ({ expand, csvFileDescriptor, expanded, close }: { close: () => void, expanded: boolean, expand: () => void, csvFileDescriptor: FileDescriptor }) => {
  ...
  return (
    <div className={`${!expanded ? "max-w-message-max" : "w-full"} !rounded !rounded-lg w-full border`}>
            ...
            <TableHeader className='!sticky !top-0 '>
              <TableRow className='!bg-neutral-100'>
                {headers.map((header, index) => (
                  <TableHead className=' !sticky !top-0 ' key={index}>{index == 0 ? "" : header}</TableHead>
                ))}
              </TableRow>
            </TableHeader>
              ...
              {data.map((row, rowIndex) => (
                <TableRow key={rowIndex}>
                  {headers.map((header, cellIndex) => (
                    <TableCell className={`${cellIndex == 0 && "sticky left-0 !bg-neutral-100"}`} key={cellIndex}>{row[header]}</TableCell>
                  ))}
                </TableRow>
              ))}
            </TableBody>
